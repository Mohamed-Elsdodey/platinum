                ...
                <div class="card-body">
                    ...
                    <table id="Datatable" class="table table-striped table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tr>
                            ...
                            <th>{{__("admin.age_from")}} </th>
                            <th>{{__("admin.age_to")}} </th>
                            <th>وقت الإضافة</th>
                            <th>التحكم</th>
                        </tr>
                        </thead>
        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">فئة الاعمار </h5>
...
@section('js')
    ...
    <script>
        ...
        datatable_selector = $('#Datatable').DataTable({
            ...
            "searching": true,
            'iDisplayLength': 20,
            "ajax": "{{route('age-ranges.index')}}",
            "columns": [
                {"data": "delete_all", orderable: false, searchable: false},
                {"data": "from", orderable: false, searchable: false},
        ...
        $(document).on('click', '#addButton', function (e) {
            e.preventDefault()
            var url = '{{route('age-ranges.create')}}';
            $.ajax({
                url: url,
                type: 'GET',
                beforeSend: function () {
        ...
        $(document).on('click', '.editButton', function (e) {
            ...
            var url = '{{route('age-ranges.edit',":id")}}';
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                console.log(result)
                if (result.value) {
                    var url = '{{ route("age-ranges.destroy", ":id") }}';
                    url = url.replace(':id', id);
                    $.ajax({
                        url: url,
                        type: 'DELETE',
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        if (id.length > 0) {
                            ...
                            $.ajax({
                                url: '{{route('age-ranges.delete.bulk')}}',
                                type: 'DELETE',
                                data: {id: id},
                                success: function (data) {
                                    $("#bulk_delete").hide()
