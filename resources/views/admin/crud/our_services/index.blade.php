...
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#addButton', function (e) {
            e.preventDefault()
            var url = '{{route('our-services.create')}}';
            $.ajax({
                url: url,
                type: 'GET',
                beforeSend: function () {
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#exampleModalLabel').html('اضافة خدمة جديد')
        ...
        $(document).on('click', '.editButton', function (e) {
            ...
            var url = '{{route('our-services.edit',":id")}}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#form-for-addOrDelete').html(data.html);
                        $('.loader-ajax').hide()
                        $('#exampleModalLabel').html('تعديل الخدمة')
                        $('#exampleModalCenter').modal('show')
                        $('.dropify').dropify()
                        $('[data-toggle="tooltip"]').tooltip()
                        $.validate({
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                console.log(result)
                if (result.value) {
                    var url = '{{ route("our-services.destroy", ":id") }}';
                    url = url.replace(':id', id);
                    $.ajax({
                        url: url,
                        type: 'DELETE',
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        if (id.length > 0) {
                            ...
                            $.ajax({
                                url: '{{route('our-services.delete.bulk')}}',
                                type: 'DELETE',
                                data: {id: id},
                                success: function (data) {
                                    $("#bulk_delete").hide()
