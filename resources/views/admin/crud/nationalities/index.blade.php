        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> الجنسيات </h5>
...
@section('js')
    ...
    <script>
        ...
        datatable_selector = $('#Datatable').DataTable({
            ...
            "searching": true,
            'iDisplayLength': 20,
            "ajax": "{{route('nationalities.index')}}",
            "columns": [
                {"data": "delete_all", orderable: false, searchable: false},
                {"data": "country_name", orderable: false, searchable: false},
        ...
        $(document).on('click', '#addButton', function (e) {
            e.preventDefault()
            var url = '{{route('nationalities.create')}}';
            $.ajax({
                url: url,
                type: 'GET',
                beforeSend: function () {
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#exampleModalLabel').html('اضافة جنسية جديد')
        ...
        $(document).on('click', '.editButton', function (e) {
            ...
            var url = '{{route('nationalities.edit',":id")}}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#form-for-addOrDelete').html(data.html);
                        $('.loader-ajax').hide()
                        $('#exampleModalLabel').html('تعديل الجنسية ')
                        $('#exampleModalCenter').modal('show')
                        $('.dropify').dropify()
                        $('[data-toggle="tooltip"]').tooltip()
                        $.validate({
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                console.log(result)
                if (result.value) {
                    var url = '{{ route("nationalities.destroy", ":id") }}';
                    url = url.replace(':id', id);
                    $.ajax({
                        url: url,
                        type: 'DELETE',
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        if (id.length > 0) {
                            ...
                            $.ajax({
                                url: '{{route('nationalities.delete.bulk')}}',
                                type: 'DELETE',
                                data: {id: id},
                                success: function (data) {
                                    $("#bulk_delete").hide()
