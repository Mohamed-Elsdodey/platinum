        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> الحالات الاجتماعية </h5>
...
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#addButton', function (e) {
            e.preventDefault()
            var url = '{{route('social-types.create')}}';
            $.ajax({
                url: url,
                type: 'GET',
                beforeSend: function () {
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#exampleModalLabel').html('اضافة الحالة الاجتماعية جديد')
        ...
        $(document).on('click', '.editButton', function (e) {
            ...
            var url = '{{route('social-types.edit',":id")}}';
            ...
            $.ajax({
                ...
                success: function (data) {
                    window.setTimeout(function () {
                        ...
                        $('#form-for-addOrDelete').html(data.html);
                        $('.loader-ajax').hide()
                        $('#exampleModalLabel').html('تعديل الحالة الاجتماعية  ')
                        $('#exampleModalCenter').modal('show')
                        $('.dropify').dropify()
                        $('[data-toggle="tooltip"]').tooltip()
                        $.validate({
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                console.log(result)
                if (result.value) {
                    var url = '{{ route("social-types.destroy", ":id") }}';
                    url = url.replace(':id', id);
                    $.ajax({
                        url: url,
                        type: 'DELETE',
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        if (id.length > 0) {
                            ...
                            $.ajax({
                                url: '{{route('social-types.delete.bulk')}}',
                                type: 'DELETE',
                                data: {id: id},
                                success: function (data) {
                                    $("#bulk_delete").hide()
