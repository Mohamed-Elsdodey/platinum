...
class WorkerFrontController extends Controller
{
    ...
    public function transport(Request $request, $type ='transport'){
        ...
        if ($country) {
            ...
            $cvs = Biography::where('status', 'new')
                ...
                ->FilterByNationality($id)->where('type','transport')
    ...
    public function showAllWorkers(Request $request, $id =null)
    {
        ...
        if ($country) {
            ...
            $cvs = Biography::where('status', 'new')
                ...
                ->FilterByAge($request->age)
                ->FilterByJob($request->job)
                ->FilterByNationality($id)
                ->with('recruitment_office', 'nationalitie', 'language_title',
                    'religion', 'job', 'social_type', 'admin', 'images', 'skills')->where('type','admission')
                ->latest()
                ->paginate(3);
        } else {
    ...
    public function show_worker_details($id,$type)
    {
